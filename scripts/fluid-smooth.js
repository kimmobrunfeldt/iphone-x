...
function getCanvasSize() {
    return {
        width: iphone.clientWidth * 3,
        height: iphone.clientHeight * 3,
    };
}
...
const displayShader = compileShader(gl.FRAGMENT_SHADER, `
    ...
    uniform sampler2D uTexture;
    void main () {
        gl_FragColor = 0.15 * texture2D(uTexture, vUv);
    }
`);
...
const advectionShader = compileShader(gl.FRAGMENT_SHADER, `
    ...
    void main () {
        ...
        gl_FragColor = 0.995 * dissipation * texture2D(uSource, coord);
    }
`);
...
function update () {
    ...
    for (let i = 0; i < pointers.length; i++) {
        ...
        if (pointer.moved) {
            splat(pointer.x, pointer.y, pointer.dx, pointer.dy, pointer.color, pointer.radius);
            pointer.moved = false;
        }
    }
    ...
    gl.uniform1f(vorticityProgram.uniforms.dt, dt * 3);
    blit(velocity.second[1]);
    velocity.swap();
...
function splat (x, y, dx, dy, color, radius) {
    ...
    gl.uniform1f(splatProgram.uniforms.radius, radius || config.SPLAT_RADIUS);
    blit(velocity.second[1]);
    velocity.swap();
...
function resizeCanvas () {
    const expectedSize = getCanvasSize();

    if (!isFloatEqual(canvas.width, expectedSize.width) || !isFloatEqual(canvas.height, expectedSize.height)) {
...
function splash(opts = {}) {
    ...
    for (var i = 0; i < 4; ++i) {
        const move = moves[i];
        pointers[i].color = move.color;
        pointers[i].x = move.x;
        pointers[i].y = move.y;
        pointers[i].dx = move.dx;
        pointers[i].dy = move.dy;
        pointers[i].down = true;
        pointers[i].moved = true;
        pointers[i].radius = opts.radius;
    }
}

function splashLoop(initial) {
    if (!initial) {
        splash();
    }
...
setTimeout(() => splash({ radius: 0.006 }), 500);
setTimeout(() => splash({ radius: 0.001 }), 2500);

function getRandomMove1() {
    const x = randomInt(0.05 * canvas.width, 0.05 * canvas.width);
    const y = randomInt(0.45 * canvas.height, 0.55 * canvas.height);
...
function getRandomMove2() {
    const x = randomInt(0.95 * canvas.width, 0.95 * canvas.width);
    const y = randomInt(0.45 * canvas.height, 0.65 * canvas.height);
...
function getRandomMove3() {
    const x = randomInt(0.95 * canvas.width, 0.95 * canvas.width);
    const y = randomInt(0.35 * canvas.height, 0.45 * canvas.height);
...
function getRandomMove4() {
    const x = randomInt(0.34 * canvas.width, 0.4 * canvas.width);
    const y = randomInt(0.8 * canvas.height, 0.8 * canvas.height);
...
function randomColor() {
    return randomChoice([
        randomizeColor([251, 6, 112]),
        randomizeColor([5, 30, 137]),
        randomizeColor([44, 94, 181]),
        randomizeColor([252, 102, 71]),
    ]);
}

function randomizeColor(rgb) {
    return [
        randomInt(rgb[0], rgb[0]),
        randomInt(rgb[1], rgb[1]),
        randomInt(rgb[2], rgb[2]),
    ];
}
