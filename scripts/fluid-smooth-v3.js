...
const displayShader = compileShader(gl.FRAGMENT_SHADER, `
    ...
    void main () {
        gl_FragColor = 0.1 * texture2D(uTexture, vUv);
    }
`);
...
function splash() {
    for (var i = 0; i < 2; ++i) {
        pointers[i].color = randomColor();
        const move = getRandomMove(i % 2 === 0, randomChoice([true, false]));
        pointers[i].x = move.x;
        pointers[i].y = move.y;
...
function splashLoop() {
    splash();
    setTimeout(splashLoop, randomInt(2000, 4000));
}
splashLoop();

function getRandomMove(isLeftSide, isUpSide) {
    const x = isLeftSide
        ? randomInt(0.1 * canvas.width, 0.2 * canvas.width)
        : randomInt(0.8 * canvas.width, 0.9 * canvas.width);

    const y = isUpSide
        ? randomInt(0.2 * canvas.height, 0.4 * canvas.height)
        : randomInt(0.6 * canvas.height, 0.8 * canvas.height);
...
function randomizeColor(rgb) {
    return [
        randomInt(rgb[0] - 40, rgb[0] + 40),
        randomInt(rgb[1] - 40, rgb[1] + 40),
        randomInt(rgb[2] - 40, rgb[2] + 10),
    ];
}
